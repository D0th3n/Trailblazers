label chapter_01:
    ...
    if chapter_01_result == "restrained":
    ...
    call anozira_evening_interlude

    call ruzen_mine_approach

    return

label chapter_01_checkpoint_evening:

    $ visited_village_exploration = True
    $ village_square_exploration_complete = True
...
label anozira_square_exploration:
    ...
    $ visited_village_exploration = True
    $ village_square_exploration_complete = False
    $ exploration_begin("anozira_square", reset_position=True)

    while not village_square_exploration_complete:
        scene bg anozira exploration
        call screen exploration_screen

        if _return == "well":
            if not explored_village_well:
                $ explored_village_well = True
...
label anozira_evening_interlude:
