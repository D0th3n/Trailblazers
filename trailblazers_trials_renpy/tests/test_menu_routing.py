...
class MenuRoutingTests(unittest.TestCase):
    ...
    def test_title_navigation_and_menu_screens_use_registry_backed_chapter_data(self):
        ...
        self.assertIn("$ chapters = chapter_card_entries()", screens_text)
        self.assertIn("for chapter in chapters:", screens_text)
        self.assertIn('$ chapter_action = Return(chapter["start_label"]) if startup_mode else Start(chapter["start_label"])', screens_text)
        self.assertIn("action chapter_action", screens_text)
